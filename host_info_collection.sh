...
function gather_ip_info () {
    ...
    local counter=0
    for ip_address in $(ip -f inet addr | grep inet | grep -Ev '127.0.0.1'\|'flannel'\|'docker'\|'virbr'\|'lo:' | awk '{if(index($2,"addr:")>0){print substr($2,6,index($2,"/")-6)}else{print substr($2,1,index($2,"/")-1)}}' | sort | uniq); do
